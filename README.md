...
중간에 **항목이 하나도 없는 주문을 내가 손에 들고 있게** 됩니다.
"주문에는 항목이 1개 이상" 이라는 규칙이 그 순간 깨져 있는 거예요.
그 상태로 다른 코드에 넘겨버릴 수도 있고요.

`Order.place()` 로 통로를 막으면 **그런 주문이 바깥으로 나갈 수 없습니다.**
**정적 팩터리는 문법이 아니라 규칙을 지키는 도구입니다.**

> **"`place()` 안에서도 `new Order()` 직후엔 항목이 0개 아닌가요?"**
> 맞습니다. 2줄 동안 있어요. 다만 그건 지역변수라 바깥의 누구도 모르고,
> `addItem` 이 실패하면 `return` 을 못 만나고 그대로 버려집니다.
> 생성자가 `private` 이라 **그 2줄을 쓸 수 있는 건 `Order` 클래스뿐**이에요.
